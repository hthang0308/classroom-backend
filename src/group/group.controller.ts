...
export class GroupController {
  ...
  async getInviteLink(@Req() req) {
    const inviteLink = await this.groupService.getInviteLink(
      req.user._id,
      req.params.id,
    );
    return {
  ...
  async inviteUser(@Req() req, @Body('email') email: string) {
    const group = await this.groupService.inviteUserViaEmail(
      req.user,
      req.params.id,
      email,
    );
  ...
  async joinGroupByInviteLink(@Req() req, @Param('token') token: string) {
    const result = await this.groupService.joinGroupByInviteLink(
      req.user,
      token,
    );
    return {
  ...
  async kickUser(@Req() req, @Query('userId') userIdToKick: string) {
    const result = await this.groupService.kickUser(
      req.user._id,
      req.params.id,
      userIdToKick,
    );
  ...
  async assignRole(@Req() req, @Body() userToUpdated: AssignRoleDto) {
    const result = await this.groupService.assignRole(
      req.user._id,
      req.params.id,
      userToUpdated,
    );
