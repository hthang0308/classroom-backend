...
export class GroupService {
  ...
  update(id: string, query: any) {
    ...
    if (!_id)
      throw new HttpException('_id is required', HttpStatus.BAD_REQUEST);
  ...
  async leaveGroup(userId: string, groupId: string): Promise<any> {
    ...
    const userAndRole = await group.usersAndRoles.find(
    ...
    if (userAndRole.role === RoleInGroup.OWNER)
      throw new HttpException(
        'Cannot leave group since you are owner. Please delete group instead',
        HttpStatus.BAD_REQUEST,
      );
  ...
  async kickUser(
    ...
    const userAndRole = await group.usersAndRoles.find(
      (item) =>
        ...
        (item.role === RoleInGroup.OWNER || item.role === RoleInGroup.CO_OWNER),
    );
    if (!userAndRole)
      throw new HttpException(
    ...
    const userAndRoleToKick = await group.usersAndRoles.find(
      (item) => item.user.toString() === userIdToKick,
    );
    if (!userAndRoleToKick)
      throw new HttpException(
        'User to kick is not a member of this group',
        HttpStatus.BAD_REQUEST,
      );
    //can not kick owner
    if (userAndRoleToKick.role === RoleInGroup.OWNER)
      throw new HttpException('Cannot kick owner', HttpStatus.BAD_REQUEST);
    await this.groupModel.updateOne(
      {
        _id: groupId,
      },
  ...
  async deleteGroup(userId: string, groupId: string): Promise<any> {
    ...
    if (!group)
      throw new HttpException('Group not found', HttpStatus.NOT_FOUND);
    if (group.userCreated.toString() !== userId)
      throw new HttpException(
    ...
    if (group.usersAndRoles.length > 1)
      throw new HttpException(
        'Cannot delete group since there are other members',
        HttpStatus.BAD_REQUEST,
      );
  ...
  async assignRole(
    userId: string,
    groupId: string,
    userToAssignRole: AssignRoleDto,
  ): Promise<any> {
    const group = await this.groupModel.findById(groupId).lean();
    if (!group)
      throw new HttpException('Group not found', HttpStatus.NOT_FOUND);
    ...
    const userAndRoleToAssign = await group.usersAndRoles.find(
      (item) => item.user.toString() === userToAssignRole.user,
    );
    if (!userAndRoleToAssign)
      throw new HttpException(
        'User to assign is not a member of this group',
        HttpStatus.BAD_REQUEST,
      );
    //can not assign owner a new role
    if (userAndRoleToAssign.role === RoleInGroup.OWNER)
      throw new HttpException('Cannot assign owner', HttpStatus.BAD_REQUEST);
  ...
  async getInviteLink(userId: string, groupId: string): Promise<any> {
    ...
    if (!group)
      throw new HttpException('Group not found', HttpStatus.NOT_FOUND);
    const userAndRole = group.usersAndRoles.find(
      (item) => item.user.toString() === userId,
    );
    if (!userAndRole)
      throw new HttpException(
  ...
  async joinGroupByInviteLink(user: any, token: string): Promise<any> {
    ...
    if (!group)
      throw new HttpException('Group not found', HttpStatus.NOT_FOUND);
    const userAndRole = group.usersAndRoles.find(
      (item) => item.user.toString() === user._id,
    );
    if (userAndRole)
      throw new HttpException(
    ...
    if (payload.emailToInvite && payload.emailToInvite !== user.email)
      throw new HttpException(
